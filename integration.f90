MODULE INTEGRATION
  USE NRUTIL, ONLY : nrerror,arth,iminloc,NR_PI,NR_EPS,NR_GAMMLN,assert_eq
  IMPLICIT NONE
...
CONTAINS
  ...
  SUBROUTINE Gauss_Laguerre(x,w,alf)
    ...
    x=z
    w=-EXP(NR_gammln(alf+n)-NR_gammln(REAL(n,8)))/(pp*n*p2)
  ...
  SUBROUTINE Gauss_Jacobi(x,w,alf,bet)
    ...
    x=z
    w=EXP(NR_gammln(alf+n)+NR_gammln(bet+n)-NR_gammln(n+1.0d0)-&
         NR_gammln(n+alf+bet+1.0d0))*temp*2.0d0**alfbet/(pp*p2)
